...
class App():
    ...
    def snapshot(self):    
        timestr = time.strftime("%Y%m%d-%H%M%S")
        cv2.imwrite(timestr + ".jpg", self.frame)
    ...
    def update(self):
        ...
        while self.analysis_switch==True:
            try:
                ...
                final = cv2.bitwise_or(gray, edged)
                final = cv2.cvtColor(final,cv2.COLOR_GRAY2RGB)

                #draw calibration line
                cv2.line(final,(int(config["p1x"]),0),(int(config["p1x"]),height),(0,255,104),1)
                cv2.line(final,(int(config["p2x"]),0),(int(config["p2x"]),height),(0,104,255),1)
                ...
                if cnts:
                    ...
                    #cv2.drawContours(final,c, 0, (0, 255, 0), 2)
                    ...
                    if dsTemp.size:
                        ...
                        cv2.circle(final, dsLeft, 5, (0, 255, 12), -1)
                        cv2.circle(final, dsRight, 5, (0, 255, 12), -1)
                    ...
                    cv2.circle(final, deLeft, 5, (0, 0, 255), -1)
                    cv2.circle(final, deRight, 5, (0, 0, 255), -1)
                    cv2.circle(final, deTop, 5, (0, 0, 255), -1)
                    cv2.circle(final, deDown, 5, (0, 0, 255), -1)
                    ...
                    if S>0 and H>0 and IFT>0:
                        cv2.rectangle(final,(10,420),(630,470),(10,160,52),-1)
                        cv2.putText(final,"DS="+str(DsMetric),(10,380), font, 1.5, (150,20,100),1, cv2.LINE_AA)
                        cv2.putText(final,"DE="+str(DeMetric),(10,410), font, 1.5, (150,20,100),1, cv2.LINE_AA)
                        cv2.putText(final,"Time="+str(datetime.datetime.now().time().strftime('%H:%M:%S')),(10,440), font, 1.5, (150,20,100),1, cv2.LINE_AA)
                        cv2.putText(final,"IFT="+str(IFT),(10,460), font, 1.5, (200,0,0),1, cv2.LINE_AA)
                        endTimer= time.time()
                        curTime="{:.2f}".format(endTimer-startTimer)
                        self.tree.insert("" , 0, values=(curTime,str(IFT)))
                        self.csvList.append([curTime,str(IFT)])
...
class sett():
    def __init__(self, window, window_title):
        ...
        self.window.geometry('%dx%d+%d+%d' % (150, 480,120,0))
